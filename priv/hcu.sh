...
echo -n "Compiling..."
COMPILE_RESULT=$(../../../../../config/rebar3 compile)
...
if [ "${IS_COMPILE_SUCCESS}" = "Compiling" ]; then
    ...
    if [ "${NEW_VSN}" = "no_change" ]; then
        ...
        ## copy, remove, and move the src folder is because the modifications which got synchronized from intellij does not
        ## work for release upgrade and the cause remains unknown, this is to workaround the issue since no other solution is found.
        cp -rf src src_bak
        rm -rf src
        mv src_bak src

        ## execute release upgrade command, version number after "-u" is the upgrade-from version.
        ../../../../../config/rebar3 release relup -u ${OLD_VSN} tar
